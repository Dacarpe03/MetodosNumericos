...
def crear_tabla(N, soporte, v_solucion, f_real):
    tabla = list()
    for i in range(N):
        if(i > 0):
            error = calcular_error(soporte[i], v_solucion[i])
        else:
            error = 0
            
        linea = [i+1, soporte[i], f_real(soporte[i]), v_solucion[i], error]
        tabla.append(linea)
    return tabla
...
+---------------------------------------------------------------------------------------------------------+\
"""
    Tabla = (Tabla.format('\n'.join("| {:<10} {:<20} {:<25} {:<25} {:<20}|".format(*fila)
                                    for fila in lista)))
    print(Tabla)
#fin mostrar_salida
...
if __name__ == "__main__":
    ...
    B = determinar_B(u_0,u_L,N)
    
    v_inicial = [0]*N
    v_solucion = metodo_Gauss_Seidel(A,B,v_inicial,0.001)
    print("\nAhora aplicamos Gauss-Seidel y obtenemos el vector solución {}".format(v_solucion))
    
    tabla = crear_tabla(N, soporte, v_solucion, f_real)
