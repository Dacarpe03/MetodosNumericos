...
def pintar_funciones(soluciones, soluciones2, soporte ,f_real, orden, nombre, h):
        y = list()
        y2 = list()
        for s in soluciones:
            y.append(s[3])
        
        for s in soluciones2:
            y2.append(s[3])
        ...
        plt.plot(soporte ,y2 , 'gv', label='Runge-Kutta orden {} con método {}'.format(orden+1, nombre))

        plt.plot(t, f_real(t), color='r', label='Función real')

        plt.title('Comparación. h={}'.format(h))
...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        solucionesRK3 = RK3(diccionario_RK3, intervalo, pasoh, solucion_inicial)
        solucionesRK4 = RK4(diccionario_RK4, intervalo, pasoh, solucion_inicial)
    
        print("Runge-Kutta de orden 3 con el método común")
        mostrar_salida(solucionesRK3)
    
    
        print("Runge-Kutta de orden 4 con el método común")
        mostrar_salida(solucionesRK4)
    
        pintar_funciones(solucionesRK3, solucionesRK4, soporte, f_real, 3, diccionario_RK4["Nombre"],pasoh)
